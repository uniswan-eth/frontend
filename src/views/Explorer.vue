      ...
      <b-row class="justify-content-center">
        <b-col lg="12">
          <card header-classes="bg-transparent" v-if="collection">
            ...
            <div class="actions" v-if="collection && this.$parent.$parent.network.chainId === 1">
              <a :href="baselink0+''" class="btn btn-sm btn-primary">
                Default
              </a>
              <a :href="baselink0+'&order=sale_date'" class="btn btn-sm btn-primary">
                Last sale
              </a>
              <a :href="baselink0+''+'order=visitor_count'" class="btn btn-sm btn-primary">
                Most viewed
              </a>
              <a :href="baselink0+'&order=sale_price'" class="btn btn-sm btn-primary">
                Buy Now
              </a>
              <a :href="baselink0+'&auction=1'" class="btn btn-sm btn-primary">
                On auction
              </a>
            </div>
            <el-table class="table-responsive table"
    ...
    methods: {
      ...
      async loadOS(offset) {
        ...
        if (address) {
          ...
          // Fetch Contract Info
          if (!this.contract) {
            res = await this.$parent.$parent.fetchOpenSea({urlcontract:true,contractaddress:address,limit:this.limit})
            this.contract = res
          }
        } else if (slug) {
        ...
        // console.log(order);
        if (order) {this.baselink += "&order="+order}
        if (buynow) {this.baselink += "&buynow=1"}
        if (auction) {this.baselink += "&auction=1"}

        if (order) {sea.order = order}
        if (offset) {sea.offset = offset}
        if (buynow || auction) {
          sea.urlorders = true
          if (buynow) {
            sea.side = 0
          ...
          if (res.orders.length < this.limit) {this.offset = -1}
          this.orders = this.orders.concat(res.orders)
        } else {
          res = await this.$parent.$parent.fetchOpenSea(sea)
          if (res.assets.length < this.limit) {this.offset = -1}
        ...
        if ((this.assets.length > 0 || this.orders.length > 0)) {
          ...
          var firstasset = this.orders.length > 0 ? this.orders[0].asset : this.assets[0]
          // Get First Asset to get full collections info
          res = await this.$parent.$parent.fetchOpenSea({urlasset:true,contractaddress:firstasset.asset_contract.address, token_id:firstasset.token_id})
