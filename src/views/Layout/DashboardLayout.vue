...
export default {
  ...
  methods: {
    ...
    async loadUser() {
      ...
      // var test = await this.getContractTokensFromSubGraph('0x36a8377e2bb3ec7d6b0f1675e243e542eb6a4764')
      // console.log('Graph', test);
    },
    async getContractTokensFromSubGraph2(contractAddress, offset = 0, limit = 5) {
      const tokensQuery = `
        {
    ...
    async getContractTokensFromSubGraph(contractAddress, startIndex = 0, amount = 5) {
      const tokensQuery = `
          query {
    ...
    async XXXdeleteOrder(order) {
      console.log('Delete order', order);
      // this.UniSwanContractV2.incrementPreferenceNonce(
      //   await this.getBundleID(pref.exchangeBundle)
      // );
    },
